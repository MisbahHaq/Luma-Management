        ...
        <AppShell breadcrumb={<span>Workspace</span>} title="My Tasks">
            <div className="max-w-7xl mx-auto space-y-4">
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2 bg-surface-2 rounded-lg p-0.5">
                        ...
                        filtered.map((task) => (
                            <div key={task.id} className="flex items-center gap-3 px-3 py-2.5 border-b border-border-subtle last:border-0 hover:bg-surface-2/50 transition-colors">
                                ...
                                <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                                    <span className="text-[10px] text-text-muted">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : '-'}</span>
                                </div>
                            </div>
                        ))
                    )}
