...
export default function NotificationsBell({ className }: NotificationsBellProps) {
    ...
    return (
        <div className="relative" data-notifications>
            <button
                onClick={() => setOpen((o) => !o)}
                className={`p-1.5 rounded hover:bg-surface-2 text-text-muted hover:text-text-secondary transition-colors relative ${className ?? ''}`}
            >
            ...
            {open && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-surface-1 border border-border-subtle rounded overflow-hidden">
